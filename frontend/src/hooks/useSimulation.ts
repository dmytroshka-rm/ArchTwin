...
export function useVersionHandshake() {
  ...
  _useEffect(() => {
    capabilitiesApi.get().then((caps) => {
      ...
      setCompatibility(
        false,
        'Could not reach backend. Check Railway (/api/health). On Vercel use frontend/vercel.json /api proxy to Railway or set VITE_API_ORIGIN and redeploy (clear build cache).',
      )
    })
  }, [setCompatibility])
}
